...
COURSE_DETAILS_MAP = {
    ...
    # General Management
    'MC':       {'Faculty': 'Arvind Singh',                'Venue': 'T3'},
...
def build_master_index(folder_path='.'):
    ...
    Scans row above headers to find correct Subject Names.
    """
    master_index = {}
    ...
    for file in files:
        try:
            with pd.ExcelFile(file) as xls:
                # Read headerless first to locate the real header
                df = pd.read_excel(xls, header=None)

            # 1. Find Header Row (Scan top 20 rows)
            header_row_idx = -1
            ...
            # 3. Process Each Block (Subject + Students)
            for roll_col in roll_col_indices:
                name_col = roll_col + 1
                
                # A. Identify Subject Name (Look at the row ABOVE the header)
                raw_section_name = ""
                if header_row_idx > 0:
                    # Scan columns around the Roll No to find the Subject Title
                ...
                # If we found a valid header inside the file, prefer that
                if clean_header:
                    # Check if it matches a known course code
                    if clean_header in [normalize_string(k) for k in COURSE_DETAILS_MAP.keys()]:
                        course_name = clean_header
                    # Or if it looks like a section name e.g. "Subject(A)"
                    elif "(" in clean_header:
                        course_name = clean_header
                
                course_name = course_name.replace("RURMKT", "RURMKT")

                # C. Extract Student Data
                ...
                for _, row in data_block.iterrows():
                    ...
                    # Handle float rolls (e.g. 463.0 -> 463)
...
def find_subjects_for_roll(target_roll, folder_path='.'):
    """
    Fast Fuzzy Search using the cached Master Index.
    """
    try:
    ...
    # 2. Fuzzy Match (Suffix)
    # Allows "463" to match "24MBA463" or "21BCM463"
